...
function Alumns({ className, cohorte }) {
  ...
  const tableData = useMemo(() => ({
    ...
    columns: [
      ...
      { key: 'email', label: 'Email', align: 'left', component: (el) => (
      <ButtonGroup>
        <Button startIcon={<MailOutlineRounded/>} href={`mailto: ${el.email}`}>Enviar</Button>
  ...
  }), [data, error, loading,copyToClipboard]);
  ...
  return (
    <div className={className}>
      <div style={{ height: '50vh', width: '100%' }}>
        <Tabla data={tableData } pageSize={5} />
      </div>
      <Snackbar open={showSnackbar} autoHideDuration={3000} onClose={()=>setShowSnackbar(false)} anchorOrigin={{vertical: 'top', horizontal:'right'}} >
        <Alert severity="info">
          Email copiado al potapapeles
        </Alert>
      </Snackbar>
    </div>
  );
